...
export function renderResult(data, fallbackText) {
  ...
  if (Array.isArray(data.wordFamily) && data.wordFamily.length) {
    const chips = data.wordFamily
      .map((it) => {
        if (!it) return '';
        // 兼容旧格式："derive (v.)"
        if (typeof it === 'string') {
          const s = it.trim();
          return s ? `<span class="ws-family-chip"><span class="w">${escapeHtml(s)}</span></span>` : '';
        }
        const w = String(it.word || '').trim();
        if (!w) return '';
        const pos = String(it.pos || '').trim();
        const zh = String(it.zh || it.meaning || '').trim();
        return (
          `<span class="ws-family-chip"><span class="w">${escapeHtml(w)}` +
          (pos ? ` <span class="p">${escapeHtml(pos)}</span>` : '') +
          `</span>` +
          (zh ? `<span class="zh">${escapeHtml(zh)}</span>` : '') +
          `</span>`
        );
      })
      .join('');
    if (chips) {
      html += '<div class="ws-section-title">词根词缀 · 派生词</div><div class="ws-family">' + chips + '</div>';
    }
  }
