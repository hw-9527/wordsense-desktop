...
export function buildPrompt(selection, context, settings) {
  ...
  parts.push(
    "5. 其他常见含义最多 3 条，常用搭配最多 4 条，例句最多 2 条（含中文翻译）。",
    "6. 若选中是单个单词，用 wordFamily 列出 2-4 个最常见的派生词或同族词，每项必须包含英文单词、词性缩写和中文释义；分析的是词组/习语或没有常见派生词时，省略该字段。",
    "",
    ...
    '  "wordFamily": [{"word": "derivation", "pos": "n.", "zh": "推导；衍生"}],',
...
export function cacheKey(text, context) {
  // v2: wordFamily 从字符串数组改为 {word,pos,zh} 对象数组，旧缓存作废
  return hashString(`v2\u0000${text}\u0000${context || ""}`);
}
