...
async function showButton() {
  ...
  // 等一帧让按钮完成布局后实测尺寸，窗口紧贴按钮（不留透明边距）
  await new Promise((r) => requestAnimationFrame(() => r()));
  const btnWidth = Math.ceil(btnWrapEl.getBoundingClientRect().width) || 110;
  const btnHeight = Math.ceil(btnWrapEl.getBoundingClientRect().height) || 52;
...
function summaryText(d) {
  ...
  if (Array.isArray(d.wordFamily) && d.wordFamily.length) {
    const parts = d.wordFamily
      .map((it) => {
        if (!it) return '';
        if (typeof it === 'string') return it.trim();
        const w = [it.word, it.pos ? `(${it.pos})` : ''].filter(Boolean).join(' ');
        return `${w} ${it.zh || it.meaning || ''}`.trim();
      })
      .filter(Boolean);
    if (parts.length) lines.push('派生词：' + parts.join(' / '));
  }
  return lines.join('\n');
}
